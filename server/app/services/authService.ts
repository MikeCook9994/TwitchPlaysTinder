...
export class AuthService {
    public static GetAuthInfo(username: string, password: string): Promise<TinderAuthInfo> {
        return AuthService.GetTinderAppAuthToken(username, password).then((authToken: string) => {
    ...
    private static GetTinderAppAuthToken(username: string, password: string): Promise<string> {
        return openBrowser().then((browser: Browser) => {
            ...
            return browser.newPage()
                ...
                .then(() => page.type('input[name=email]', username))
